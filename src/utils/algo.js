export function markWords(answer, solution) {
  ...
  const dp = Array.from({ length: answerWords.length + 1 }, () =>
    Array(solutionWords.length + 1).fill(0)
  );
  ...
  while (i > 0 && j > 0) {
    if (answerWords[i - 1] === solutionWords[j - 1]) {
      markedAnswer.unshift(
        `<span style="color:green">${answerWords[i - 1]}</span>`
      );
      i--;
      j--;
    } else if (dp[i - 1][j] >= dp[i][j - 1]) {
      markedAnswer.unshift(
        `<span style="color:red">${answerWords[i - 1]}</span>`
      );
      i--;
    } else {
      j--;
